...
const AdminLayout: React.FC = () => {
  ...
  const [openSections, setOpenSections] = useState<Record<string, boolean>>(() => {
    try {
      return JSON.parse(localStorage.getItem(LS_SECTIONS) ?? '') ?? { pipeline: true, content: true, social: true };
    } catch {
      return { pipeline: true, content: true, social: true };
    }
  });
  ...
  <>
    <div className="px-3 py-3 flex items-center justify-between">
      <Link to="/admin" onClick={onClose} className="flex items-center gap-2">
        ...
        <span className="text-[14px] font-medium text-[#37352F]">Homie CMS</span>
      </Link>
